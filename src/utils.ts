...
export const validateUniswapPair = async (tokenPoolAddress: string) => {
    try {
        ...
        if (uniV3Factory.getPool(pairPool.token0(), pairPool.token1(), pairPool.fee()) == tokenPoolAddress.toLowerCase() && pairPool.factory() == UNIV3_FACTORY_ADDRESS.toLowerCase()){
            return true;
        } else {
            return false;
        }
...
export const generateFinding = (eventName: string, sender: string, recipient: string, amount0: number, amount1: number, tokenPoolAddress: string) : Finding => {
